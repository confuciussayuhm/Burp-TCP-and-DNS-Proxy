...
public class GenericMiTMServer
		...
		try {
			...
			timer.scheduleAtFixedRate(new TimerTask() {
				...
				public void run() {
					...
					// System.out.println("Listener " + ListenPort + " has " +pairs.size() + "
					// threads running");
			...
			while (true && !killme) {
				try {
					...
					System.out.println(connectionSocket.getPort() + " :: " + connectionSocket.getLocalPort() + " :: "
							+ pairs.size());
					SendData send = new SendData(this, true, isSSL); // bug... changed from false
					/*
					 * if(pairs.size() != 0){ send = (SendData)pairs.keySet().toArray()[0]; }else{
					 */
					...
					/*
					 * if(pairs.size() != 0){ getD = ((SendData)pairs.keySet().toArray()[0]).doppel;
					 * }else{
					 */
	...
	private void KillSocks(SendData sd) {
		...
		try {
			if (sd.isSSL()) {
				/*
				 * ((SSLSocket)sd.sock).shutdownInput(); ((SSLSocket)sd.sock).shutdownOutput();
				 */
				((SSLSocket) sd.sock).close();
			} else {
				/*
				 * ((Socket)sd.sock).shutdownInput(); ((Socket)sd.sock).shutdownOutput();
				 */
	...
	public void Closed(SendClosedEvent e) {
		synchronized (this) {
			 ...
			 * ((SSLSocket)tmp.sock).getPort()); System.out.println("There are " +
			 * pairs.size() + " number of threads."); for(SendData s : pairs.keySet()){
