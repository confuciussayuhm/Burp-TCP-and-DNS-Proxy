...
public class DynamicKeyStore {
	...
	public X509Certificate createCert(PublicKey pubKey, PrivateKey intPrivKey, PublicKey intPubKey,
			X509Certificate intCert, String cn, String o, String ou, String l, String st, String c)
		...
		/*
		 * X509v3CertificateBuilder certBuilder = new X509v3CertificateBuilder(new
		 * X500Name("CN="+cn+", OU="+ou+", O="+o+", L="+l),BigInteger.valueOf(new
		 * SecureRandom().nextLong()), new Date(), expiry.getTime(), new
		 * X500Name("CN=PortSwigger CA"),
		 * SubjectPublicKeyInfo.getInstance(pubKey.getEncoded())); byte[] certBytes =
		 * certBuilder.build(new JCESigner(intPrivKey, "SHA256withRSA")).getEncoded();
		 * CertificateFactory certificateFactory =
		 * CertificateFactory.getInstance("X.509"); X509Certificate certificate =
		 * (X509Certificate)certificateFactory.generateCertificate(new
		 * ByteArrayInputStream(certBytes)); return certificate;
		 */
		 ...
		 * AuthorityKeyIdentifierStructure(intPubKey));
		 */
		/*
	...
	public X509Certificate createItermCert(PublicKey pubKey, PrivateKey caPrivKey, X509Certificate caCert, String cn,
			String o, String ou, String l, String st, String c) throws InvalidKeyException, IllegalStateException,
			NoSuchProviderException, NoSuchAlgorithmException, SignatureException, CertificateException {
		final int daysTillExpiry = 10 * 365;
	...
	public String generateKeyStore(String password, String cn) {
		try {
			...
			/*
			 * for(String p : dirs){ System.out.println(p);
			 * 
			 * /*String fp = p.substring(0, p.lastIndexOf(fs)); //String f = fp + fs +
			 * "MiTMKS4.jks"; String f = fp + fs + "burpca.p12"; //String f =
			 * "/Applications/burp/burpca.p12"; System.out.println(f);
			 * if(p.contains("burpca.p12")){ JKS=p; filefound=true;
			 * 
			 * } }
			 */
			if (!filefound) {
			...
			chain[1] = cacert;
			chain[0] = (Certificate) createItermCert(intkeyPair.getPublic(), caPrivKey, (X509Certificate) cacert, cn,
					"PortSwigger", "PortSwigger", "PortSwigger", "PortSwigger", "PortSwigger");
