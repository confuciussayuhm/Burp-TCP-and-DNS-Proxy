...
public class LogEntry {
	...
	public LogEntry(byte[] requestResponse, byte[] original, String SrcIp, int SrcPort, String DstIP, int DstPort,
			String Direction) {
	...
	public LogEntry(Long Index, String SrcIp, int SrcPort, String DstIP, int DstPort, String Direction, Long time,
			int bytes) {
		this.Index = Index;
	...
	public long save() {
		...
		Requests dao = new Requests(0, this.requestResponse, this.original, this.SrcIP, this.SrcPort, this.DstIP,
				this.DstPort, this.Direction, this.time.getTime(), this.Bytes);
	...
	public long saveCommitDeferred(LogEntry le) {
		Requests dao = new Requests(0, le.requestResponse, le.original, le.SrcIP, le.SrcPort, le.DstIP, le.DstPort,
				le.Direction, le.time.getTime(), le.Bytes);
		session.save(dao);
		this.Index = (long) dao.getId();
		return dao.getId();
	}
	...
	public Requests getData(Long index) {
		...
		try {
			...
			r = (Requests) s.createQuery("from Requests where id = :id").setInteger("id", index.intValue())
					.uniqueResult();
	...
	public static LinkedList<LogEntry> restoreDB() {
		...
		for (Requests q : r) {
			list.add(new LogEntry((long) q.getId(), q.getSrcIp(), q.getSrcPort(), q.getDstIp(), q.getDstPort(),
					q.getDirection(), q.getDate(), q.getBytes()));
			// list.add(new LogEntry(q.getData(), q.getOriginal(), (long)q.getId(),
			// q.getSrcIp(), q.getSrcPort(), q.getDstIp(), q.getDstPort(),
			// q.getDirection(),q.getDate(), q.getBytes()));
		}
		s.close();
		return list;
	...
	public static LinkedList<LogEntry> searchDB(String query) {
		...
		List<Requests> r = (List<Requests>) s
				...
				.setParameter("term", "%" + query + "%").setParameter("term2", "%" + query + "%").list();
		LinkedList<LogEntry> list = new LinkedList<LogEntry>();
		for (Requests q : r) {
			list.add(new LogEntry((long) q.getId(), q.getSrcIp(), q.getSrcPort(), q.getDstIp(), q.getDstPort(),
					q.getDirection(), q.getDate(), q.getBytes()));
