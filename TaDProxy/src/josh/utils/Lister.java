			...
			PacketListener listener = new PacketListener() {
				...
				public void gotPacket(PcapPacket packet) {
					...
					if (tcp.getHeader().getSyn() && !tcp.getHeader().getAck()
							&& !ip.getHeader().getSrcAddr().toString().equals("/" + IP)) {
						// System.out.println(ip.getHeader().getSrcAddr() + " : " +
						// tcp.getHeader().getDstPort().toString() );
						...
						fireEvent(ip.getHeader().getSrcAddr().getHostAddress(), tcp.getHeader().getDstPort().name(),
								tcp.getHeader().getDstPort().valueAsInt(), time);
					}
				}
			};
