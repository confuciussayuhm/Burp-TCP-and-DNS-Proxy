	...
	public void run() {
		try {
			...
			while ((le = queue.poll()) != null) {
				Requests dao = new Requests(0, le.requestResponse, le.original, le.SrcIP, le.SrcPort, le.DstIP,
						le.DstPort, le.Direction, le.time.getTime(), le.Bytes);
	...
	public void run3() {
		try {
			...
			while ((le = queue.poll()) != null) {
				...
				stmt.executeUpdate();
				stmt.close();
				/// Requests dao = new Requests(0, le.requestResponse, le.original, le.SrcIP,
				/// le.SrcPort, le.DstIP, le.DstPort, le.Direction, le.time.getTime(),
				/// le.Bytes);
				// session.save(dao);
				// le.Index =(long)dao.getId();
				// le.save();
